...
/** Redeemable code for a full monthly Pro subscription (no Lemon Squeezy). */
...
export async function grantManualMonthlyProSubscription(userId: string): Promise<void> {
  ...
  const billingSubscriptionId = `sub_manual_${userId}`
  const billingCustomerId = `cus_manual_${userId}`
  ...
  await creditCitesOnce({
    ...
    kind: 'subscription',
    referenceId: grantReference(billingSubscriptionId, periodStart),
